...
rate = tf.placeholder(tf.float32,shape=(1))
...
inter2 = tf.multiply(rate,inter1)
	...
	with tf.Session() as sess:
		...
		#for step in range(0,5000):				
			...
			if str(list1) == str(list2):
				...
				err += 1
		print("Before any SGD error " + str(float(err)/len(arrays_validate)))
		#print("Before any SGD error " + str(float(err)/5000))
		print("Before any SGD loss " + str(loss_itr))
		...
		while True:
			...
			for step in range(0,5000):
				...
				lr  =  np.array([learning_rate + (0.01/count)])
				start_time = time.time()
				sess.run([update_grads,weights],feed_dict={toClassify:arrays_train[step], correctChoice: correct,length:arrays_train[step].shape,rate:lr})
				end_time   = time.time()
				#print("Time for processing each tuple is " + str(end_time-start_time))
			epoch_end_time = time.time()
			print("Epoch Time is " + str(epoch_end_time - epoch_start_time))
			save_path = saver.save(sess, "../data/models/model_full_incr.ckpt")
			print("Model Saved")
