		...
		with tf.Session(server.target,config=config) as sess:
			...
			while True:
				...
				for step in range(0,100):
					...
					val = sess.run([testValidating])
					#while master is validating do nothing
					while val[0] == 1.0:
						val = sess.run([testValidating])
					start_time = time.time()
					print("Start time for iteration " + str(iteration) + " is " + str(start_time))
				...
				if FLAGS.task_index == 0:
					save_path = saver.save(sess, "../data/models/model_parallel_4.ckpt")
					print("Model Saved")
